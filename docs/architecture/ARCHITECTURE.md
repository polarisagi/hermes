...
- **系统级常量**：在 `internal/domain/provider.go` 中定义强类型字符串常量，确保 Go 层类型安全，数据库层保持高可读性：
  ...
  ```
- **智能 Token 提取机制**：网关在处理 `AuthTypeADC` 或其他鉴权时，会智能识别用户在 `adc_json` 等字段中填入的凭据类型。若提供的是完整的 Service Account / ADC JSON 格式文件，则走标准 ADC 流程（使用 `golang.org/x/oauth2/google` 自动完成 OAuth Token 置换与基于内存 `sync.Map` 的续期缓存）；若仅为一段非 JSON 的纯文本串（例如通过 UI 填入的 OAuth Access Token 或 API Key），则网关会自动退化并作为标准的 Bearer Token 或对应 Header 进行注入，实现无缝兼容。
